        ...
        function o() {
            ...
            if ("object" == typeof e) {
                ...
                var ll = false,
                    ...
                    add, s2_ = s;
                if (1 === n) {
                    "set" === (e[0].toString().match(/^function\s*([^\s(]+)/) || [])[1] ? (p = e[1][1], d = e[1][1][2], ll = "∞" === l, /*""+p[2][5]!==""+s2?(""+p[2][5].search("j")==-1 ?p[2][5]="j":s2):"m" ,*/ (p[2][3][4] == s2 && p[2][0].length === s.length && p[2][0] === s) ? (p[2][3][5] > 0 && (p[2][3][5]--), "0" === p[2][0][p[2][0].length - 1] ? ( /*console.log(".0 + " + p[2][0]),*/ p[2][0] = s = "0", p[2][3][3]++ /*, console.log("0 + " + p[2][0])*/ ) : ( /*console.log(".1 - " + p[2][0]),*/ p[2][0] = s = "1", p[2][3][3]-- /*, console.log("1 - " + p[2][0])*/ ), "function" == typeof p[2][4] ? ( (p[2][4].toString().match(/^function\s*([^\s(]+)/) || [])[1]=="ho2_"?(a =p[2][4](e,p[2][1], d[2][0]),a[2] != undefined && (a[2]=="cancel"&&(clearInterval(i),return void),a[2]!="∞"?g = a[2]-1:g = a[2]),add =  [p[1],p[2][3][1]]):(a = p[2][4](p[2][1], d[2][0], p[1], d[3][1], e[1][0]), a[2] != undefined && (add = a[2])),p[2][3][0]==="∞"&&(p[2][3][3]=0), p[2][1] = a[0], d[2][0] = a[1]) : "+" === p[2][1] && (ll || d[2][0] < d[3][0] ) ? d[2][0]++ : (d[2][0]--, p[2][1] = "-", -1 === d[2][0] && (d[2][0] += 2, p[2][1] = "+")), e[0](e[1][0], p[0], add != undefined ? add[0] : p[1], d[2])) : (p[2][3][5]--, clearInterval(i), returnBo = false)) : e[0]("obj", e[1]);
//g=d[3][0];
                     //console.log(g)
                   /*
             if(!returnBo){
